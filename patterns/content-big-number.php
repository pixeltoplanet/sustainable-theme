<?php

/**
 * Title: Content big number
 * Slug: sustainable-theme/content-big-number
 * Categories: sustainable-theme,sustainable-theme/content
 * Description: Content section with oversized number focus.
 * Keywords: content, number, big, typography
 * Inserter: true
 */

?>
<!-- wp:group {"align":"full","style":{"spacing":{"padding":{"top":"var:preset|spacing|fluid-x-large","bottom":"var:preset|spacing|fluid-x-large"}}},"backgroundColor":"neutral-1","layout":{"type":"constrained"}} -->
<div class="wp-block-group alignfull has-neutral-1-background-color has-background" style="padding-top:var(--wp--preset--spacing--fluid-x-large);padding-bottom:var(--wp--preset--spacing--fluid-x-large)"><!-- wp:columns {"align":"wide","verticalAlignment":"center","style":{"spacing":{"blockGap":{"top":"var:preset|spacing|fluid-large","left":"var:preset|spacing|fluid-large"}}}} -->
<div class="wp-block-columns alignwide are-vertically-aligned-center"><!-- wp:column {"width":"30%","verticalAlignment":"center"} -->
<div class="wp-block-column is-vertically-aligned-center" style="flex-basis:30%"><!-- wp:paragraph {"align":"center","style":{"typography":{"fontSize":"clamp(5rem, 15vw, 12rem)","fontWeight":"700","lineHeight":"0.9"}}} -->
<p class="has-text-align-center" style="font-size:clamp(5rem, 15vw, 12rem);font-weight:700;line-height:0.9">01</p>
<!-- /wp:paragraph -->
</div>
<!-- /wp:column -->

<!-- wp:column {"width":"70%","verticalAlignment":"center","style":{"spacing":{"blockGap":"var:preset|spacing|fluid-small"}}} -->
<div class="wp-block-column is-vertically-aligned-center" style="flex-basis:70%"><!-- wp:heading {"level":2,"fontSize":"xl"} -->
<h2 class="wp-block-heading has-xl-font-size">We start with the why</h2>
<!-- /wp:heading -->

<!-- wp:paragraph {"fontSize":"lg"} -->
<p class="has-lg-font-size">Before we design a single pixel, we invest time understanding your business, your audience, and what success looks like. This foundation shapes everything that follows.</p>
<!-- /wp:paragraph -->
</div>
<!-- /wp:column -->
</div>
<!-- /wp:columns -->
</div>
<!-- /wp:group -->
